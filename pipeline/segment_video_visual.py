...
def process_video(
    ...
    Args:
        ...
        output_video: Path to save visualization video (None = don't save)
    """
    ...
    # Get original frame dimensions (use from frame if different from metadata)
    frame_height, frame_width = first_frame.shape[:2]
    if original_width == 0 or original_height == 0:
        original_width, original_height = frame_width, frame_height
    
    print(f"Video dimensions: {original_width}x{original_height}, FPS: {fps}")
    
    # Initialize video writer if output_video is specified
    video_writer = None
    if output_video:
        ...
        if not video_writer.isOpened():
            print(f"Warning: Could not initialize video writer for {output_video}")
            video_writer = None
        else:
            print(f"Saving visualization video to: {output_video}")
    ...
    while True:
        ...
        # Create visualization frame (for display and/or video output)
        vis_frame = None
        if display or video_writer:
            vis_frame = frame.copy()
            if result.masks is not None and len(result.masks.data) > 0:
                mask = result.masks.data[0].cpu().numpy()
                if len(mask.shape) == 2:
                    if mask.shape != vis_frame.shape[:2]:
                        mask = cv2.resize(
                            mask.astype(np.uint8),
        ...
        if video_writer and vis_frame is not None:
...
Examples:
  ...
  python segment_video.py video.mp4 --object-name car --width 640 --display
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to save visualization video (shows bounding box, mask overlay, and contours)"
    )
