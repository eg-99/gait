"""
Training script for GEI Denoising Variational Autoencoder (DVAE)

Trains DVAE to learn robust probabilistic latent space by reconstructing
clean images from corrupted inputs.

Usage:
    python train_dvae.py --epochs 50 --batch_size 32 --lr 0.001 --beta 1.0 --noise-type gaussian
"""
...
from dvae_model import create_dvae, dvae_loss


def parse_args():
    parser = argparse.ArgumentParser(description='Train GEI Denoising VAE')
    ...
    # Checkpointing
    parser.add_argument('--save_dir', type=str, default='checkpoints_dvae',
                       help='Directory to save model checkpoints')
    parser.add_argument('--log_dir', type=str, default='logs_dvae',
...
def train_epoch(model, dataloader, optimizer, device, epoch, beta):
    ...
    for batch_idx, (gei, _, _) in enumerate(pbar):  # Unpack (data, label, metadata)
        ...
        # Forward pass with corruption (DVAE)
        reconstruction, mu, log_var, corrupted = model(gei, corrupt=True)
        
        # Compute loss - compare reconstruction to CLEAN target
        total_loss, recon_loss, kl_loss = dvae_loss(
            reconstruction, gei, mu, log_var, corrupted=corrupted, beta=beta
        )
...
def save_checkpoint(model, optimizer, epoch, train_losses, val_losses, save_path, noise_type, noise_std, noise_prob):
...
def plot_training_curves(history, save_path):
    ...
    axes[0].set_title('Total DVAE Loss', fontsize=14)
    ...
    axes[1].set_title('Denoising Loss (MSE)', fontsize=14)
...
def main():
    ...
    # Create directories (separate from regular VAE)
